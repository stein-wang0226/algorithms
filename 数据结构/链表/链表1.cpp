...
for(int i=1;i<=n;i++){
  ...
  cin>>t;
  if(t>0){
    if(list1.phead->next==NULL){//空
      addtail(t,&list1);
      continue;
    }
    Node *temp=list1.phead->next;
    Node*pos=list1.phead;
    while(temp->data<t){//找最后一个比t小的
      pos=temp;
      temp=temp->next;
      if(temp==NULL)break;
       }
if(pos==list1.phead)addhead_(t,&list1);//插在头上
else if(pos->next!=NULL)add_to_pos(t,pos,&list1);// 插在中间
else addtail(t,&list1);//插在末尾
  }
  else if(t<0){
     if(list2.phead->next==NULL){//空
      addtail(t,&list2);
      continue;
    }
...
if(temp==list2.phead->next)addhead_(t,&list2);//插在头上
else if(temp->next!=NULL)add_to_pos(t,temp,&list2);// 插在中间
else addtail(t,&list2);//插在末尾
}
}
output(&list1);
cout<<endl;
