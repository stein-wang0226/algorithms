...
利用记忆化搜索，从高位搜到低位，满足条件跳出返回1，否则返回0

用limit参数控制上界，end表示是否到当前可能最大值( int end = limit ? b[pos] : 9)   ，limit limit false 才向后转移

dp[] [] 初值-1

dp[ pos] [state]  记录1-pos位(1~10^i中)满足state的总数        （  无上界限制），用来存储降低运算次数

pos 为当前搜到的位数（从高位到低位），state 储存状态 ---可多维(多位数或多条件)

state 条件  eg  上一位为4的后pos位总数
...
int dfs(int pos, int state, bool limit) {  //位置(从高位到低位)    状态state:前缀到该位是否含有13 (存前缀状态)
                                            //可多个   limit:是否达到上界_有限制 ：前面每一位都达到上界  （与n的前若干位相同）   这样后面每一位都限制上界  不然可以取遍1~9    
  if (pos == 0)   
  ...
  int ans = 0;
  for (int i = 0; i <= end; i++) {  // 枚举 0-end    含0注意判断下开头0
    if ()  // 满足某种条件
      ans += dfs(pos - 1, state, limit && i == end);//   limit每位都为最高位
  }
  if (!limit) dp[pos][state] = ans;  //没有限制才转移（后面每位都可取0~9）  不然无法准确判断 -- 可能与state条件矛盾
  return ans;
    LL solve(LL n)
{
	int cnt=0;
	...
	return dfs(cnt-1,-1,false,true); // 第一位肯定是有上界的  取决于n的第一位
}
...
//  到着做  算  N-不含的
const int N = 2e5 + 5;
int T, n, dp[30][2];  //dp[i][0/1]  表示1~i位(0-10^i-1)的所有数中满足不含49个数（无限制（1~i=10^i-1））  第二维表示该位是否位为4
int b[30];// 每一位数字
int dfs(int pos, bool is_4, bool limit) {  //位置(从高位到低位)    状态state     is_4上一位是4(也可直接传数)
                                            //可多个   limit:前缀
    是否达到上界_有限制 ：前面每一位都达到上界  （与n的前几位相同）   这样后面每一位都限制上界  不然可以取遍1~9    
  if (pos == 0) 
    return 1;  //能到0位的都算一种   但多算一个0 最后减去
  if (!limit && ~dp[pos][is_4]) return dp[pos][is_4]; //  没有限制才转移（1~9）
  int end = limit ? b[pos] : 9;  //该位最大值
  int ans = 0;
  for (int i = 0; i <= end; i++) {//枚举pos位(下一位)
    if (!(is_4&&i==9))  // 满足某种条件 （直到pos不含49）
...
//正着做 含49 需要多一维state ：因为只要前面有49出现就会向后传递
//   dp[pos][pre][state]为第pos位上，前一个数(pos位)为pre，目前状态为state（所枚举的这个数字是否含有相邻的49）时的数字数量。
// 
...
LL dfs(int pos,int pre,bool state,bool limit)
{
	...
	for(int i=0;i<=up;i++) 
	{
	ans+=dfs(pos-1,i,state||i==9&&pre==4,limit&&i==b[pos]); //当前是或之前是都要算上
	}
	if(!limit)
...
int dfs(int pos, int mod1,int mod2, bool state,bool limit) {  //位置(从高位到低位)  到该位每一位求和对7取模   之前组成的数模  到该位是否含7  限制 
  ...
  if (pos == 0){//最后一位跳出 
        if(state||mod1==0||mod2==0)  return 1;  //满足条件   mod 的数论特性
        else return 0;
  }
